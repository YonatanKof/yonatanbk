	...
	<div class="client-section">
		...
		<div class="logo-container" ref="containerRef">
			<div class="logo-scroll" :style="scrollStyle">
				...
				<!-- Second set of logos -->
				<img v-for="logo in logos" :key="`clone1-${logo}`" :src="logo" :alt="getLogoName(logo)" class="client-logo" />
				<!-- Third set of logos -->
				<img v-for="logo in logos" :key="`clone2-${logo}`" :src="logo" :alt="getLogoName(logo)" class="client-logo" />
			</div>
		</div>
	</div>
</template>
